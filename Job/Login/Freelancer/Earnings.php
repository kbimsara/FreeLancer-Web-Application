<?php
$page = "Profile";
$btn = "";
require_once './action/action-session.php';
?>

<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">

    <!-- Page Title bar -->
    <link rel="icon" type="image/png" href="./assets/logo/titlebar_logo.png" />

    <!-- Bootstrap -->
    <link href="./assets/bootstrap-4.6.2/dist/css/bootstrap.min.css" rel="stylesheet" type="text/css" />
    <script src="./assets/bootstrap-4.6.2/dist/js/bootstrap.min.js" type="text/javascript"></script>

    <title>Home</title>
    <!-- Custom css -->
    <link rel="stylesheet" href="./assets/style.css">
</head>

<body style="background-color: #dadada;">
    <!-- Nav -->
    <?php
    require_once './Page-ex/nav.php';
    ?>
    <div class="container-fluid mn" style="padding: 0px; margin: 0px;">
        <!-- Back -->
        <div class="container-fluid" style="padding: 0px;padding-left: 5px; margin: 0px;">
            <span class="section-title">
                <button class="btn btn-light back-btn" onclick="history.back()">
                    <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="currentColor" class="bi bi-arrow-left-circle" viewBox="0 0 16 16">
                        <path fill-rule="evenodd" d="M1 8a7 7 0 1 0 14 0A7 7 0 0 0 1 8zm15 0A8 8 0 1 1 0 8a8 8 0 0 1 16 0zm-4.5-.5a.5.5 0 0 1 0 1H5.707l2.147 2.146a.5.5 0 0 1-.708.708l-3-3a.5.5 0 0 1 0-.708l3-3a.5.5 0 1 1 .708.708L5.707 7.5H11.5z" />
                    </svg>
            </span>
        </div>
        <div class="row justify-content-center" style="margin: 0px;">
            <?php
            require_once './action/config.php';

            $seller_id = $_SESSION["seller_id"];
            $query_search = "SELECT * FROM `orders` WHERE seller_id='$seller_id';";
            $query_run = mysqli_query($Connector, $query_search);
            $t = 0;
            $pd = 0;
            $pdc = 0;
            $cm = 0;
            $c = 0;
            $i = 0;
            $n = 0;

            while ($row = mysqli_fetch_array($query_run)) {
                $order_id  = $row['order_id'];
                $user_id = $row['user_id'];
                $seller_id = $row['seller_id'];
                $pk = $row['pk'];
                $pk_short = $row['pk_short'];
                $price = $row['price'];
                $qt = $row['qt'];
                $est_time = $row['est_time'];
                $order_status = $row['order_status'];
                $tot = $row['tot'];
                $t = $t + $price;
                if ($order_status == "complete") {
                    $cm = $cm + $price;
                    $i++;
                } else if ($order_status == "ongoin") {
                    $pd = $pd + $price;
                    $n++;
                } else if ($order_status == "Cancel") {
                    $c = $c + $price;
                } else {
                    $pdc = $pdc + $price;
                }
            }
            $year = date("Y");
            $month = date("m");
            $date1=$year."-".$month."-01";
            $date2=$year."-".$month."-".date("d");
            //in monthly income
            $query_search2 = "SELECT COUNT(user_id) AS id,SUM(price) AS tot FROM `orders` WHERE seller_id='$seller_id' AND `od_date` BETWEEN $date1 AND $date2;";
            $query_run2 = mysqli_query($Connector, $query_search2);
            $c2=0;
            $sum=0;

            while ($row = mysqli_fetch_array($query_run2)) {
                $cnt_id  = $row['id'];
                $sum = $row['tot'];
            }
            ?>


            <div class="col-xl-10 col-12">
                <div class="row">
                    <div class="col-12" style="text-align: center;">
                        <h3 class="clr">LKR <?php echo $t; ?>.00</h3>
                        <p>Total Earning</p>
                    </div>
                </div>
            </div>
            <div class="col-xl-10 col-12" style="margin: 0px; padding: 0px;">
                <div class="row justify-content-center">
                    <div class="col-12 col-xl-6">
                        <div class="row">
                            <div class="col-12">
                                <h4>Analytics</h4>
                            </div>
                        </div>
                        <div class="row m-2">
                            <div class="col-12 c-card-2 bg-white">
                                <div class="row m-1">
                                    <div class="col-7">
                                        <h6>Earnings in This Month</h6>
                                    </div>
                                    <div class="col-5" style="text-align: right;">
                                        <h6 class="clr">LKR <?php echo $sum; ?></h6>
                                    </div>
                                </div>
                                <div class="row m-1">
                                    <div class="col-7">
                                        <h6>Avg. selling Price</h6>
                                    </div>
                                    <div class="col-5" style="text-align: right;">
                                        <h6 class="clr">LKR <?php echo ($t)."/".($sum); ?></h6>
                                    </div>
                                </div>
                                <div class="row m-1">
                                    <div class="col-7">
                                        <h6>Active Oders</h6>
                                    </div>
                                    <div class="col-5" style="text-align: right;">
                                        <h6 class="clr">(<?php echo $n; ?>) LKR <?php echo $pd; ?></h6>
                                    </div>
                                </div>
                                <div class="row m-1">
                                    <div class="col-7">
                                        <h6>Completed Oders</h6>
                                    </div>
                                    <div class="col-5" style="text-align: right;">
                                        <h6 class="clr">(<?php echo $i; ?>) LKR <?php echo $cm; ?></h6>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-12 col-xl-6">
                        <div class="row">
                            <div class="col-12">
                                <h4>Revenues</h4>
                            </div>
                        </div>
                        <div class="row m-2">
                            <div class="col-12 c-card-2 bg-white">
                                <div class="row m-1">
                                    <div class="col-7">
                                        <h6>Cancelled Orders</h6>
                                    </div>
                                    <div class="col-5" style="text-align: right;">
                                        <h6 class="clr">LKR <?php echo $c; ?></h6>
                                    </div>
                                </div>
                                <div class="row m-1">
                                    <div class="col-7">
                                        <h6>Pending Clearance</h6>
                                    </div>
                                    <div class="col-5" style="text-align: right;">
                                        <h6 class="clr">LKR <?php echo $pd; ?></h6>
                                    </div>
                                </div>
                                <div class="row m-1">
                                    <div class="col-7">
                                        <h6>Cleared</h6>
                                    </div>
                                    <div class="col-5" style="text-align: right;">
                                        <h6 class="clr">LKR <?php echo $cm; ?></h6>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

        </div>
    </div>

    <!-- Sticky bar -->
    <?php
    require_once './Page-ex/menu-bar.php';
    ?>
    </div>

</body>
<!-- Body/End -->

<script src="https://code.jquery.com/jquery-3.3.1.slim.min.js" integrity="sha384-q8i/X+965DzO0rT7abK41JStQIAqVgRVzpbzo5smXKp4YfRvH+8abtTE1Pi6jizo" crossorigin="anonymous"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.6/umd/popper.min.js" integrity="sha384-wHAiFfRlMFy6i5SRaxvfOCifBUQy1xHdJ/yoi7FRNXMRBu5WHdZYu1hA6ZOblgut" crossorigin="anonymous"></script>
<script src="https://stackpath.bootstrapcdn.com/bootstrap/4.2.1/js/bootstrap.min.js" integrity="sha384-B0UglyR+jN6CkvvICOB2joaf5I4l3gm9GU6Hc1og6Ls7i6U/mkkaduKaBhlAXv9k" crossorigin="anonymous"></script>


</html>