...
<body>
    ...
    //check rated ids
    $gig_count=0;
    $query_searchrt = "SELECT COUNT(orders.order_id ) AS cc FROM rate JOIN orders ON rate.gig_id=orders.gig_id WHERE orders.order_id='$order_id';";
    $query_runrt = mysqli_query($Connector, $query_searchrt);

    while ($row = mysqli_fetch_array($query_runrt)) {
        $gig_count = $row['cc'];
    }
    ...
    if (isset($_POST['confirm'])) {
        ...
        $query_update = "UPDATE `orders` SET `order_status` = 'complete' WHERE `orders`.`order_id` = '$order_id';";
        ...
        if ($query_update_run) {
            echo "<script>Swal.fire({
                                ...
                                window.location = './ongoin-3.php?order_id=$order_id';
                            })</script>";
        } else {
            echo "<script>Swal.fire({
                                icon: 'error',
                ...
                <div class="col-xl-10 col-11">
                    ...
                    <div class="row justify-content-center">
                        <div class="col-12 justify-content-center" style="text-align: center;">
                            ...
                            if ($gig_count == 0) {
                            ?>
                                <a href="./ongoin-4.php?gig_id=<?php echo $gig_id; ?>">
                                    <button type="button" class="btn btn-warning m-1">Rate</button>
                                </a>
                            <?php
                            } else {
                            ?>
                                <button type="button" class="btn btn-warning m-1" disabled>Rated</button>
                            <?php
                            }
                            ?>
                        </div>
